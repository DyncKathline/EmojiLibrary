...
public class EmojiCharacterUtil {
    ...
    public static String escape(String src) {
        ...
        for (int index = firCodeIndex; index <= lstCodeIndex;) {
            ...
            if (isEmojiCharacter(codepoint)) {
                ...
                sb.append(separator).append(hash.length()).append(unicode_prefix).append(unicode_separator).append(hash);
    ...
    public static void emoji2bytes(String src){
        if (TextUtils.isEmpty(src)) {
            return;
        }
        int cpCount = src.codePointCount(0, src.length());
        int firCodeIndex = src.offsetByCodePoints(0, 0);
        int lstCodeIndex = src.offsetByCodePoints(0, cpCount - 1);
        StringBuilder sBuffer = new StringBuilder(src.length());
        ...
        try {
            ...
            for (byte b : utf8Decode) {
                sBuffer.append("\\x").append(Integer.toHexString(b & 0xFF));
            }
    ...
    public static String reverse(String src) {
        ...
        int index = 0;//ab:5u+1f60d:4u+de0da
        while (index < sourceChar.length) {
            if (sourceChar[index] == separator) {
                ...
                if (sourceChar[index + 1] >= '4' && sourceChar[index + 1] <= '6' && sourceChar[index + 2] == unicode_prefix && sourceChar[index + 3] == unicode_separator) {
                ...
                } else if (sourceChar[index + 1] == separator) { // 通用字符的反转
